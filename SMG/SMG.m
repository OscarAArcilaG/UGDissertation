function SMG(varargin)
    ...
    global M GP FP TP ZZ TN GG SP ...
           miniaturePath temporalWindowPath aboutIconPath ...
           userManualPath licenseAgreementPath fileAssociationPath
           
    % MATLAB Compiler don't support addpath
    addpath(genpath(pwd));
    
    if isdeployed
        [~, result] = system('path');
        installPath = char(regexpi(result,'Path=(.*?);','tokens','once'));
        miniaturePath = fullfile(installPath,'miniature.jpg');
        temporalWindowPath = fullfile(installPath,'temporalWindow.jpg');
        aboutIconPath = fullfile(installPath,'icon48.jpg');
        userManualPath = fullfile(installPath,'SMGUserManual.pdf');
        fileAssociationPath = fullfile(installPath,'SMGFileAssociation.pdf');
        licenseAgreementPath = fullfile(installPath,'SMGLicenseAgreement.pdf');
    else
        miniaturePath = fullfile(pwd,'Images','miniature.jpg');
        temporalWindowPath = fullfile(pwd,'Images','temporalWindow.jpg');
        aboutIconPath = fullfile(pwd,'Images','icon48.jpg');
        userManualPath = fullfile(pwd,'Docs','SMGUserManual.pdf');
        fileAssociationPath = fullfile(pwd,'Docs','SMGFileAssociation.pdf');
        licenseAgreementPath = fullfile(pwd,'Docs','SMGLicenseAgreement.pdf');
    end
    ...
    elseif ~isempty(varargin)
        if (length(varargin) == 1)
            ...
            if exist(fileName, 'file') == 2
                msg1 = msgbox(['Processing the file: ', fileName]);
                ...
                msg2 = msgbox(['Project loaded: ', fileName]);
                jframe = get(msg2,'javaframe');
                jframe.setFigureIcon(javax.swing.ImageIcon(miniaturePath));
            else
                msg3 = msgbox(['The file does not exist: ', fileName]);
